...
window.addEventListener("load", () => {
  ...
  debugBox.innerHTML = "🧪 TESTE INICIADO...<br>";
  ...
  log("confirmBuyer: " + (typeof confirmBuyer));
  log("toggle: " + (typeof toggle));
  log("openModal: " + (typeof openModal));
  log("markAsPending: " + (typeof markAsPending));
  log("sendToWhatsApp: " + (typeof sendToWhatsApp));
  log("saveData: " + (typeof saveData));
  log("loadData: " + (typeof loadData));
  log("renderPanel: " + (typeof renderPanel));
  log("checkExpiredPendings: " + (typeof checkExpiredPendings));

  // ===============================
  // Testar se elementos HTML existem
  // ===============================

  log("<br>🔍 Verificando elementos do HTML...");
  ...
  const paymentAreaEl = document.getElementById("paymentArea");
  const gridEl = document.getElementById("grid");

  log("buyerName: " + (buyerNameEl ? "OK" : "❌ NÃO ACHOU"));
  log("buyerPhone: " + (buyerPhoneEl ? "OK" : "❌ NÃO ACHOU"));
  log("modal: " + (modalEl ? "OK" : "❌ NÃO ACHOU"));
  log("buyerForm: " + (buyerFormEl ? "OK" : "❌ NÃO ACHOU"));
  log("paymentArea: " + (paymentAreaEl ? "OK" : "❌ NÃO ACHOU"));
  log("grid: " + (gridEl ? "OK" : "❌ NÃO ACHOU"));

  // ===============================
  // Testar localStorage
  // ===============================

  log("<br>🔍 Testando localStorage...");

  try {
    localStorage.setItem("testeLocal", "123");
    const test = localStorage.getItem("testeLocal");
    log("localStorage: OK (" + test + ")");
  } catch (e) {
    log("localStorage: ❌ ERRO -> " + e.message);
  }

  // ===============================
  // Testar raffleData carregado
  // ===============================

  log("<br>🔍 raffleData atual...");

  try {
    if (typeof raffleData !== "undefined") {
      log("raffleData length: " + raffleData.length);
    } else {
      log("raffleData: ❌ NÃO EXISTE");
    }
  } catch (e) {
    log("raffleData erro: " + e.message);
  }

  // ===============================
  // Teste manual do botão
  // ===============================

  log("<br>🟠 Teste do botão confirmBuyer...");

  try {
    if (typeof confirmBuyer === "function") {
      log("confirmBuyer existe ✔");
    } else {
      log("confirmBuyer NÃO existe ❌");
    }
  } catch (e) {
    log("confirmBuyer erro: " + e.message);
  }
